    ...
    */
    ...
    public required init?(coder aDecoder: NSCoder) {
        ...
        if let image = self.image {
            buttonFrame = CGRect(x: 0.0, y: 0.0, width: image.size.width+50, height: image.size.height+50)
        } else if let title = self.title {
            let lbl = UILabel()
            lbl.text = title
            let textSize = lbl.sizeThatFits(CGSize(width: 50, height: 50))
...
fileprivate extension BadgedBarButtonItem {
    ...
    func createBadgeLabel() -> UILabel {
        ...
        label.textColor =  UIColor.white//badgeProperties.textColor
        label.font = badgeProperties.font
        label.backgroundColor = UIColor.red //badgeProperties.backgroundColor
    ...
    func shouldBadgeHide(_ value: String) -> Bool {
        return (value == "") && shouldHideBadgeAtZero
    }
