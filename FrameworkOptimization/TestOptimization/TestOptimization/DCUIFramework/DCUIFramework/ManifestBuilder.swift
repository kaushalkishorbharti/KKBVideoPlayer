            ...
            if line.isEmpty {
                ...
                if line.hasPrefix("#EXTM3U") {
                    ...
                    do {
                      ...
                      var resolutionString = line
                        let arrList = line.characters.split{$0 == ","}.map(String.init)
                      arrList.forEach { (str) in
                        if str.hasPrefix("RESOLUTION") {
                          let resolutionList = str.characters.split{$0 == "x"}.map(String.init)
                          resolutionString = resolutionList.last ?? ""
                          
                        }
                      }
                      
                        // let resolutionString = try line.replace("(.*),(.*),(.*)=(\\d+)x(\\d+),(.*)", replacement: "$5")
                        ...
                        if let currentMediaPlaylistExist = currentMediaPlaylist {
                            if let iProgramIdString = Int(programIdString) {
                                currentMediaPlaylistExist.programId = iProgramIdString
                            }
                            if let iBandwidthString = Int(bandwidthString) {
                                currentMediaPlaylistExist.bandwidth = iBandwidthString
                            }
                            if(resolutionString == line){
                                DCUIFramework.language = (UserDefaults.standard.object(forKey: "currentLanguage") as?String)!
                                 currentMediaPlaylistExist.resolution = "Audio"
                                if DCUIFramework.language == "vi" {
                                  currentMediaPlaylistExist.resolution = "Chỉ có audio"
                                }
                            } else {
