...
open class INSPhotoViewController: UIViewController, UIScrollViewDelegate {
    ...
    private func loadFullSizeImage() {
        ...
        self.photo.loadImageWithCompletionHandler({ [weak self] (image, error) -> () in
            let completeLoading = {
                ...
                if( (self?.photo as! INSPhoto).fileType == .FileTypeDocument){
                    
                     self?.addPdfViewButton();
                }
                
                if( (self?.photo as! INSPhoto).fileType == .FileTypeAudio){
                    
                    self?.addAudioPlayerButton();
                }
    ...
    open func addPdfViewButton(){
        let view:UIView = UIView.init(frame: CGRect.init(x: 0, y: 0, width: 100.0, height: 100.0))
        
        view.backgroundColor = UIColor.init(red: 0, green: 0, blue: 0, alpha: 0.75)
        view.layer.cornerRadius = 8;
        view.clipsToBounds = true;
        
        let image:UIImageView = UIImageView.init(image: DCUIFImageView.ICON_VIEW_PDF_IMAGE)
        image.frame = CGRect.init(x: 30, y: 18, width: 40.0, height: 40.0)
        ...
        label.frame = CGRect.init(x: 8, y: 66, width: 84, height: 20.0)
        label.text = "Tap to view".localized(withComment: "");
    ...
    open func addAudioPlayerButton()
    {
     
        let view:UIView = UIView.init(frame: CGRect.init(x: 0, y: 0, width: 40.0, height: 40.0));
        
        view.backgroundColor = UIColor.clear;
        view.clipsToBounds = true;
        
        let image:UIImageView = UIImageView.init(image: UIImage.init(named: "play-audio", in: DCUIFramework.bundle, compatibleWith: nil)?.withRenderingMode(.alwaysOriginal))
        image.frame = CGRect.init(x: 0, y: 0, width: 40.0, height: 40.0)
        image.backgroundColor = UIColor.clear;
