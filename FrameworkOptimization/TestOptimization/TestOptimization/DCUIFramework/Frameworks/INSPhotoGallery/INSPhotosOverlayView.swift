...
open class INSPhotosOverlayView: UIView , INSPhotosOverlayViewable {
    ...
    private func setupNavigationBar() {
        ...
        if let bundlePath = Bundle(for: type(of: self)).path(forResource: "INSPhotoGallery", ofType: "bundle") {
            ...
            leftBarButtonItem = UIBarButtonItem.init(image: UIImage.init(named: "icon-cross-white", in: DCUIFramework.bundle, compatibleWith:nil), style: UIBarButtonItemStyle.plain, target: self, action: #selector(INSPhotosOverlayView.closeButtonTapped(_:)))
