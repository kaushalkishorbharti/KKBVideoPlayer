...
extension PageViewItem {
    ...
    fileprivate func createCircleLayer(_ radius: CGFloat, lineWidth: CGFloat) -> CAShapeLayer {
        ...
        let path = UIBezierPath(roundedRect: CGRect(x: 0, y: 0, width: 20, height: 2), cornerRadius: 0)
...
//        return imageView
        ...
        let imageView = Init(UIImageView(frame: CGRect(x: 0, y: 0, width: 20, height: 2))) {
            $0.contentMode = .scaleAspectFit
            $0.translatesAutoresizingMaskIntoConstraints = false
            $0.alpha = select == true ? 1 : 0
        }
        //Changes in Paper onbording
        imageView.backgroundColor = UIColor.init(red: 239/255, green: 56/255, blue: 76/255, alpha: 1.0)
        addSubview(imageView)
