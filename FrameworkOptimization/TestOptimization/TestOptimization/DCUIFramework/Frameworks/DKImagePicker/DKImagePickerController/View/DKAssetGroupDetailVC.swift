...
open class DKAssetGroupDetailVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, DKGroupDataManagerObserver {
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if self.isCameraCell(indexPath: indexPath) {
            ...
            if let ast = (collectionView.cellForItem(at: indexPath) as? DKAssetGroupDetailBaseCell)?.asset {

                
               
                let manager = PHImageManager.default()
                manager.requestAVAsset(forVideo: ast.originalAsset!, options: nil) { (avasset, audio, info) in

                    if let avassetURL = avasset as? AVURLAsset {
                        let data = NSData(contentsOf: avassetURL.url as URL)!
                       
                        if (Double(data.length/(DCUIFTheme.sharedInstance.videoMaxlimit * 1048576)) > 1){
                        
                            self.removeAsset(collectionView, removeItemAt: indexPath);
                            DCUIFAlert.showOkAlert(title: "Alert", description: "Your file is larger than \(DCUIFTheme.sharedInstance.videoMaxlimit) mb. Please upload a file less than \(DCUIFTheme.sharedInstance.videoMaxlimit) mb. ", onViewCotroller: DCUIFViewController.getTopViewController());
    ...
    public func removeAsset(_ collectionView: UICollectionView, removeItemAt indexPath: IndexPath){
