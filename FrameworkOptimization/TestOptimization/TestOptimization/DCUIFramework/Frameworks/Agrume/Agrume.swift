...
public final class Agrume: UIViewController {
  ...
  public var statusBarStyle: UIStatusBarStyle? {
    ...
    private func addRotateButton(){
        let deviceType = DCUIDeviceType.current.deviceType
        rotateButton = UIButton.init();
        
        if "\(deviceType)".contains("iPhoneX") {
            rotateButton.frame = CGRect.init(x: UIScreen.main.bounds.width - 80, y: 40, width: 40, height: 40)
        }else {
            rotateButton.frame = CGRect.init(x: UIScreen.main.bounds.width - 80, y: 20, width: 40, height: 40)
        }
        
        rotateButton.setImage(DCUIFImageView.ROTATE_SCREEN_IMAGE, for: UIControlState.normal);
        rotateButton.addTarget(self, action: #selector(rotateScreen), for: UIControlEvents.touchUpInside)
        rotateButton.layer.cornerRadius = rotateButton.frame.size.width/2;
        rotateButton.backgroundColor = UIColor.init(red: 0/255, green: 0/255, blue: 0/255, alpha: 0.6);
        rotateButton.translatesAutoresizingMaskIntoConstraints = false
        
        rotateButton.clipsToBounds = true;
        
//        if "\(deviceType)".contains("iPhoneX") {
//            button.center = CGPoint.init(x: UIScreen.main.bounds.width - 60, y: 60)
//        }else {
//            button.center = CGPoint.init(x: UIScreen.main.bounds.width - 60, y: 40)
//        }
  ...
  // Add Close Button
    private func addCloseButton() {
        ...
        if "\(deviceType)".contains("iPhoneX") {
            button.frame = CGRect.init(x: 40, y: 40, width: 40, height: 40)
        }else {
            button.frame = CGRect.init(x: 20, y: 20, width: 40, height: 40)
        }

        button.setImage(DCUIFImageView.CROSS_IMAGE, for: UIControlState.normal);
        button.addTarget(self, action: #selector(dismissAfterTap), for: UIControlEvents.touchUpInside)
        button.layer.cornerRadius = button.frame.size.width/2;
        button.backgroundColor = UIColor.init(red: 0/255, green: 0/255, blue: 0/255, alpha: 0.6);
        button.translatesAutoresizingMaskIntoConstraints = false
    
        button.clipsToBounds = true;
        
//        if "\(deviceType)".contains("iPhoneX") {
//            button.center = CGPoint.init(x: 60, y: 60)
//        }else {
//            button.center = CGPoint.init(x: 40, y: 40)
//        }
  ...
  private var backgroundImageView: DCUIFImageView!
  ...
  override public func viewDidLoad() {
   ...
   // backgroundImageView = UIImageView(frame: view.frame)
    backgroundImageView = DCUIFImageView(frame: view.frame)
  ...
  public override func viewWillAppear(_ animated: Bool) {
    ...
    public override func viewWillDisappear(_ animated: Bool) {
        ...
        let potraitMode = UIInterfaceOrientation.portrait.rawValue
        UIDevice.current.setValue(potraitMode, forKey: "orientation")
        lastUsedOrientation = currentDeviceOrientation()
    }
